...
void NoiseGenerator::setParametars(const NoiseParameters &params) noexcept {
...
double NoiseGenerator::noise(double x, double z) const noexcept {
    auto floorX = (double) ((int) x); //This is kind a cheap way to floor a double integer.
    ...
    auto rec2 = lerp(u, v, x - floorX); //Here we use x-floorX, to get 1st dimension. Don't mind the x-flooX thingie, it's part of the cosine foumula.
