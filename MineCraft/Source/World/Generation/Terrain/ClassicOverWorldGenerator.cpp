#include "ClassicOverWorldGenerator.hpp"
...
#include "../../../Maths/GeneralMaths.hpp"

#include "../Structures/TreeGenerator.hpp"
...
ClassicOverWorldGenerator::ClassicOverWorldGenerator()
        : m_grassBiome(seed), m_temperateForest(seed), m_desertBiome(seed), m_oceanBiome(seed), m_lightForest(seed) {
    setUpNoise();
}
...
void ClassicOverWorldGenerator::setUpNoise() {
    ...
    if (!noiseGen) {
        ...
        m_biomeNoiseGen.setParametars(biomeParams);
    }
}
